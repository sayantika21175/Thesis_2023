...
r=10;
df = r*(d1+d2-r);
m1 = min(5*df,round(.99*d1*d2) ); 
p  = m1/(d1*d2);
delta1=5;%5 for cv1  15 for cv2
delta = delta1/p; 
maxiter = 500; 
tol = 1e-4;
[U,S,V,numiter]=SVT(n,IDX,y,tau,delta,maxiter,tol);
[Yhat]=U*S*V';
...
function Yhat=alg_bmc(matrix,~,~,test_ind,~)
...
%My code for GRDMF%
function Yhat=alg_grdmf(matrix,Sd,St,test_ind,~)
%k1=25;
%k2=20;
global k1 k2 p
...
%mu1=0.09;mu2=0.01; % for cv3
%mu1=0.01;mu2=0.0001;
%mu1=0.06;mu2=1.6; % for cv2
global mu1 mu2
...
global lamda mu1 mu2 pp nu1 nu2;
%nu1 nu2;
...
function [S,p]=preprocess_PNN_asc(S,p)
%preprocess_PNN sparsifies S by keeping, for each drug/virus, the "p"
% nearest neighbors (NNs) and discarding the rest. 

    NN_mat = zeros(size(S));
    for j=1:length(NN_mat)
        [~,indx] = sort(S(j,:),'ascend');
        indx = indx(1:p+1);     % keep drug/virus j and its "p" NNs
        NN_mat(j,indx) = 1;
    end
    NN_mat = (NN_mat+NN_mat')/2;
